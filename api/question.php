...
function _parseParagraphArray(array $lines): array {
    ...
    $questions = [];
    $i         = 0;
    $lastProcessed = -1; // indeks baris terakhir yang sudah jadi bagian soal

    /* ---- helper closures ---- */

    // Opsi PERTAMA: dimulai dengan أ. <spasi> atau A. / A)
    $isFirstOpt = static function (string $l): bool {
        // أ atau ا diikuti titik + spasi
        if (preg_match('/^[\x{0623}\x{0627}]\.\s/u', $l)) return true;
        // Latin A diikuti titik/kurung/spasi
        if (preg_match('/^[Aa][).\s]/u', $l)) return true;
        return false;
    };

    // Opsi LANJUTAN: ب ت ث ج atau B C D E
    $isNextOpt = static function (string $l): bool {
        if (preg_match('/^[\x{0628}\x{062A}\x{062B}\x{062C}][.\s]/u', $l)) return true;
        if (preg_match('/^[B-Eb-e][).\s]/u', $l)) return true;
        return false;
    };

    // Strip label dari baris opsi
    $stripLabel = static function (string $l): string {
        $l = preg_replace('/^[\x{0600}-\x{06FF}]\.\s*/u', '', $l);
        $l = preg_replace('/^[A-Ea-e][).\s]+/', '', $l);
        return trim($l);
    };
    ...
    while ($i < $total) {
        ...
        /* == STRATEGI 1: label Arab / Latin (أ. / A.) == */
        if ($isFirstOpt($line)) {
            // Kumpulkan teks soal dari baris sebelumnya (max 5, tidak melewati soal yang sudah diproses)
            $qParts = [];
            for ($j = $i - 1; $j > $lastProcessed && $j >= max(0, $i - 5); $j--) {
                $prev = $lines[$j];
                if ($isFirstOpt($prev) || $isNextOpt($prev)) break;
                array_unshift($qParts, $prev);
            }

            // Kumpulkan opsi mulai dari $i
            $opts = [];
            $k    = $i;
            while ($k < $total && count($opts) < 5) {
                $ol = $lines[$k];
                if (count($opts) === 0 && !$isFirstOpt($ol)) break;
                if (count($opts) >  0 && !$isNextOpt($ol))  break;
                $text = $stripLabel($ol);
                if ($text !== '') {
                    $opts[] = $makeOpt($text, count($opts));
                }
                $k++;
            }

            if (!empty($qParts) && count($opts) >= 2) {
                $questions[] = [
        ...
        /* == STRATEGI 2: soal tanpa label (ends with .... / ? / ؟) == */
        // Hanya jika baris ini belum diproses sebagai bagian soal lain
        if ($i > $lastProcessed && preg_match('/[.…]{2,}\s*$|[\x{061F}?]\s*$/u', $line) && ($i + 1) < $total) {
            $candidates = [];
            for ($k = $i + 1; $k < $total && count($candidates) < 5; $k++) {
                $nl = $lines[$k];
                // Hentikan jika soal baru / label opsi / terlalu panjang
                if (preg_match('/[.…]{2,}\s*$|[\x{061F}?]\s*$/u', $nl)) break;
                if ($isFirstOpt($nl)) break;
                if (strlen($nl) > 120) break;
                $candidates[] = $nl;
            }
            ...
            if (count($candidates) >= 3) {
                ...
                $lastProcessed = $i + count($candidates);
                $i = $lastProcessed + 1;
                continue;
            }
        }
    ...
    // Fallback jika terlalu sedikit soal
