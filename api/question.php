...
function _parseParagraphArray(array $lines): array {
     ...
     * Deteksi: baris berakhir .... / ? / ؟ / !
     *          diikuti 3-5 baris pendek yang belum dipakai
     * ────────────────────────────────────────────────────────── */
    $pass2 = [];
    $i = 0;
    while ($i < $total) {
        ...
        for ($k = $i + 1; $k < $total && count($cands) < 5; $k++) {
            ...
            if (strlen($nl) > 120) break;                         // terlalu panjang = wacana
            $cands[] = $nl;
        }
        if (count($cands) < 3) { $i++; continue; }

        $opts = array_map(fn($c,$idx)=>$makeOpt($c,$idx), $cands, array_keys($cands));

        // Ambil konteks sebelum soal (max 2 baris, tidak dipakai)
        $ctx = [];
        for ($j = $i - 1; $j >= max(0, $i - 3) && !isset($used[$j]); $j--) {
            if ($isQEnd($lines[$j]) || $arabIdx($lines[$j]) >= 0) break;
            array_unshift($ctx, $lines[$j]);
        }
